...
function handlevar(mqttmsg, mqttpayload) {
	...
	if (mqttmsg.match(/^openwb\/graph\//i)) {
		processGraphMessages(mqttmsg, mqttpayload);
	} else if (mqttmsg.match(/^openwb\/evu\//i)) {
		processEvuMessages(mqttmsg, mqttpayload);
	} else if (mqttmsg.match(/^openwb\/global\/awattar\//i)) {
		processETProviderMessages(mqttmsg, mqttpayload);
	} else if (mqttmsg.match(/^openwb\/global\/ETProvider\//i)) {
		processETProviderMessages(mqttmsg, mqttpayload);
		...
		processVerbraucherMessages(mqttmsg, mqttpayload);
	} else if (mqttmsg.match(/^openwb\/lp\//i)) {
		processLpMessages(mqttmsg, mqttpayload);
	} else if (mqttmsg.match(/^openwb\/hook\//i)) {
		processHookMessages(mqttmsg, mqttpayload);
	} else if (mqttmsg.match(/^openwb\/SmartHome\/Devices\//i)) {
		processSmartHomeDevicesMessages(mqttmsg, mqttpayload);
	} else if (mqttmsg.match(/^openwb\/config\/get\/SmartHome\/Devices\//i)) {
		processSmartHomeDevicesConfigMessages(mqttmsg, mqttpayload);
...
function processETProviderMessages(mqttmsg, mqttpayload) {
	...
	if (mqttmsg == "openWB/global/ETProvider/providerName") {
		...
		$(".etproviderLink").attr(
			"href",
			"/openWB/modules/" + mqttpayload + "/stromtarifinfo/infopage.php"
		);
		...
		electricityPriceTimeline = getCol(csvData, 0).map(function (x) {
			return x * 1000;
		});
		...
		$("#aktuellerStrompreis").text(
			parseFloat(mqttpayload).toLocaleString(undefined, {
...
function processEvuMessages(mqttmsg, mqttpayload) {
	...
	if (mqttmsg == "openWB/evu/W") {
		...
		if (powerEvu > 999) {
			powerEvu = (powerEvu / 1000).toLocaleString(undefined, {
		...
		if (evuiDailyYield >= 0) {
			var evuiDailyYieldStr =
				" (" +
				evuiDailyYield.toLocaleString(undefined, {
					minimumFractionDigits: 2,
					maximumFractionDigits: 2,
				}) +
				" kWh I)";
		...
		if (evueDailyYield >= 0) {
			var evueDailyYieldStr =
				" (" +
				evueDailyYield.toLocaleString(undefined, {
					minimumFractionDigits: 2,
					maximumFractionDigits: 2,
				}) +
				" kWh E)";
...
function processGlobalMessages(mqttmsg, mqttpayload) {
	...
	if (mqttmsg == "openWB/global/WHouseConsumption") {
		...
		if (powerHouse > 999) {
			powerHouse =
				(powerHouse / 1000).toLocaleString(undefined, {
					minimumFractionDigits: 2,
					maximumFractionDigits: 2,
				}) + " kW";
		} else {
		...
		if (powerAllLp > 999) {
			powerAllLp =
				(powerAllLp / 1000).toLocaleString(undefined, {
					minimumFractionDigits: 2,
					maximumFractionDigits: 2,
				}) + " kW";
		} else {
		...
		if (llaDailyYield >= 0) {
			var llaDailyYieldStr =
				" (" +
				llaDailyYield.toLocaleString(undefined, {
					minimumFractionDigits: 2,
					maximumFractionDigits: 2,
				}) +
				" kWh)";
		...
		if (hausverbrauchDailyYield >= 0) {
			var hausverbrauchDailyYieldStr =
				" (" +
				hausverbrauchDailyYield.toLocaleString(undefined, {
					minimumFractionDigits: 2,
					maximumFractionDigits: 2,
				}) +
				" kWh)";
...
function processHousebatteryMessages(mqttmsg, mqttpayload) {
	...
	if (mqttmsg == "openWB/housebattery/W") {
		...
		if (speicherwatt > 999) {
			speicherwatt = (speicherwatt / 1000).toLocaleString(undefined, {
				minimumFractionDigits: 2,
				maximumFractionDigits: 2,
			});
			unit = " kW";
		...
		if (sieDailyYield >= 0) {
			var sieDailyYieldStr =
				" (" +
				sieDailyYield.toLocaleString(undefined, {
					minimumFractionDigits: 2,
					maximumFractionDigits: 2,
				}) +
				" kWh E)";
		...
		if (siiDailyYield >= 0) {
			var siiDailyYieldStr =
				" (" +
				siiDailyYield.toLocaleString(undefined, {
					minimumFractionDigits: 2,
					maximumFractionDigits: 2,
				}) +
				" kWh I)";
...
function processPvMessages(mqttmsg, mqttpayload) {
	...
	if (mqttmsg == "openWB/pv/W") {
		...
		if (pvwatt <= 0) {
			...
			if (pvwatt > 999) {
				pvwatt =
					(pvwatt / 1000).toLocaleString(undefined, {
						minimumFractionDigits: 2,
						maximumFractionDigits: 2,
					}) + " kW";
			} else {
		...
		if (pvDailyYield >= 0) {
			var pvDailyYieldStr =
				" (" +
				pvDailyYield.toLocaleString(undefined, {
					minimumFractionDigits: 2,
					maximumFractionDigits: 2,
				}) +
				" kWh)";
...
function processVerbraucherMessages(mqttmsg, mqttpayload) {
	...
	if (mqttmsg.match(/^openwb\/Verbraucher\/[1-2]\/Configured$/i)) {
		...
		if (verbraucherwatt > 999) {
			verbraucherwatt = (verbraucherwatt / 1000).toLocaleString(undefined, {
				minimumFractionDigits: 2,
				maximumFractionDigits: 2,
			});
			unit = " kW";
		...
		mqttmsg.match(/^openwb\/Verbraucher\/[1-2]\/DailyYieldImportkWh$/i)
	) {
		...
		if (verbraucherDailyYield >= 0) {
			var verbraucherDailyYieldStr =
				" (" +
				verbraucherDailyYield.toLocaleString(undefined, {
					minimumFractionDigits: 2,
					maximumFractionDigits: 2,
				}) +
				" kWh)";
...
function processLpMessages(mqttmsg, mqttpayload) {
	...
	if (mqttmsg.match(/^openwb\/lp\/[1-9][0-9]*\/w$/i)) {
		...
		if (actualPower > 999) {
			actualPower = (actualPower / 1000).toLocaleString(undefined, {
				minimumFractionDigits: 2,
				maximumFractionDigits: 2,
			});
			actualPower += " kW";
		...
		if (mqttpayload == 1) {
			element
				...
				.addClass("text-green animate-alertPulsation");
		} else {
			element
				.removeClass("text-green animate-alertPulsation")
				.addClass("text-orange");
		}
	} else if (mqttmsg.match(/^openwb\/lp\/[1-9][0-9]*\/strchargepointname$/i)) {
		var index = getIndex(mqttmsg); // extract number between two / /
		$(".nameLp").each(function () {
			// fill in name for all element of class '.nameLp'
		...
		var index = getIndex(mqttmsg); // extract number between two / /
		$(".nameLp").each(function () {
			// check all elements of class '.nameLp'
			var lp = $(this).closest("[data-lp]").data("lp"); // get attribute lp from parent
			if (lp == index) {
...
function processHookMessages(mqttmsg, mqttpayload) {
	...
	if (mqttmsg.match(/^openwb\/hook\/[1-9][0-9]*\/boolhookstatus$/i)) {
		...
		mqttmsg.match(/^openwb\/hook\/[1-9][0-9]*\/boolhookconfigured$/i)
	) {
		var index = getIndex(mqttmsg); // extract number between two / /
		if (mqttpayload == 1) {
...
function processSmartHomeDevicesMessages(mqttmsg, mqttpayload) {
	...
	if (mqttmsg.match(/^openwb\/SmartHome\/Devices\/[1-9][0-9]*\/Watt$/i)) {
		...
		mqttmsg.match(/^openwb\/SmartHome\/Devices\/[1-9][0-9]*\/DailyYieldKwh$/i)
	) {
		var index = getIndex(mqttmsg); // extract number between two / /
		var parent = $('[data-dev="' + index + '"]'); // get parent row element for SH Device
		...
		if (actualDailyYield >= 0) {
			var actualDailyYieldStr =
				" (" +
				actualDailyYield.toLocaleString(undefined, {
					minimumFractionDigits: 2,
					maximumFractionDigits: 2,
				}) +
				" kWh)";
		...
		mqttmsg.match(
			/^openwb\/SmartHome\/Devices\/[1-9][0-9]*\/RunningTimeToday$/i
		)
	) {
		var index = getIndex(mqttmsg); // extract number between two / /
		var parent = $('[data-dev="' + index + '"]'); // get parent row element for SH Device
		...
		mqttmsg.match(/^openwb\/SmartHome\/Devices\/[1-9][0-9]*\/Status$/i)
	) {
		var index = getIndex(mqttmsg); // extract number between two / /
		$(".nameDevice").each(function () {
			// check all elements of class '.nameLp'
			...
			if (dev == index) {
				if ($(this).hasClass("enableDevice")) {
					...
					if (mqttpayload == 10) {
						$(this)
							.removeClass("lpEnabledStyle")
							.removeClass("text-blue")
							.removeClass("text-white")
							.removeClass("lpWaitingStyle")
							.addClass("lpDisabledStyle");
						// 11 Device off(manuell oder automatisch)
					} else if (mqttpayload == 11) {
						$(this)
							.removeClass("lpDisabledStyle")
							.removeClass("text-blue")
							.removeClass("text-white")
							.removeClass("lpWaitingStyle")
							.addClass("lpEnabledStyle");
		...
		mqttmsg.match(
			/^openwb\/SmartHome\/Devices\/[1-9][0-9]*\/TemperatureSensor0$/i
		)
	) {
		var index = getIndex(mqttmsg); // extract number between two / /
		...
		if (isNaN(actualTemp)) {
			...
			if (actualTemp > 200) {
				...
				StringTemp =
					"Temp1 " +
					actualTemp.toLocaleString(undefined, {
						minimumFractionDigits: 2,
						maximumFractionDigits: 2,
					}); // make complete string to display
		...
		mqttmsg.match(
			/^openwb\/SmartHome\/Devices\/[1-9][0-9]*\/TemperatureSensor1$/i
		)
	) {
		var index = getIndex(mqttmsg); // extract number between two / /
		...
		mqttmsg.match(
			/^openwb\/SmartHome\/Devices\/[1-9][0-9]*\/TemperatureSensor2$/i
		)
	) {
		var index = getIndex(mqttmsg); // extract number between two / /
		var parent = $('.SmartHomeTemp[data-dev="' + index + '"]'); // get parent row element for charge point
...
function processSmartHomeDevicesConfigMessages(mqttmsg, mqttpayload) {
	...
	if (
		mqttmsg.match(
			/^openwb\/config\/get\/SmartHome\/Devices\/[1-9][0-9]*\/device_configured$/i
		)
	) {
		...
		mqttmsg.match(
			/^openwb\/config\/get\/SmartHome\/Devices\/[1-9][0-9]*\/mode$/i
		)
	) {
		var index = getIndex(mqttmsg); // extract number between two / /
		var parent = $('[data-dev="' + index + '"]'); // get parent row element for SH Device
		...
		$(".nameDevice").each(function () {
			// check all elements of class '.nameDevice'
		...
		mqttmsg.match(
			/^openWB\/config\/get\/SmartHome\/Devices\/[1-9][0-9]*\/device_name$/i
		)
	) {
		var index = getIndex(mqttmsg); // extract number between two / /
		var parent = $('[data-dev="' + index + '"]'); // get parent row element for SH Device
